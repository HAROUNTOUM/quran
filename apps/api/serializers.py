...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if validated_data.get("role") in (User.Role.ADMIN, User.Role.SUPERVISOR):
            validated_data["is_approved"] = User.ApprovalStatus.APPROVED
...
class CircleDetailSerializer(CircleListSerializer):
    ...
    class Meta(CircleListSerializer.Meta):
        fields = CircleListSerializer.Meta.fields + ["description", "surah_range", "enrolled_students"]
...
class CircleCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Circle
        fields = [
            "name", "teacher_id", "description", "surah_range",
...
class CircleRemoveStudentSerializer(serializers.Serializer):
    ...
    def save(self, **kwargs):
        circle = self.context["circle"]
        enrollment = CircleEnrollment.objects.get(
            circle=circle,
...
class RecitationGradeCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if att and att.status == Attendance.Status.ABSENT:
...
class ReviewRequestCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = ReviewRequest
        fields = ["circle", "type", "surah", "ayah_from", "ayah_to", "notes"]
